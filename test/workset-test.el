...
;;;; workset-worktree--parse-wt-entry tests

(ert-deftest workset-test-parse-wt-entry ()
  "Parse a wt list JSON entry into a plist."
  (let* ((commit (make-hash-table :test #'equal))
         (wt (make-hash-table :test #'equal))
    ...
    (let ((result (workset-worktree--parse-wt-entry entry)))
      ...
      (should (equal (plist-get result :path) "/tmp/wt/feature"))
      (should (equal (plist-get result :kind) "worktree"))
      (should (equal (plist-get result :commit-sha) "abc123def456"))
      (should (equal (plist-get result :commit-short-sha) "abc123d"))
      (should (equal (plist-get result :commit-message) "Fix bug"))
      (should (equal (plist-get result :commit-timestamp) 1700000000))
      (should (equal (plist-get result :main-state) "ahead"))
      (should (equal (plist-get result :main-ahead) 3))
      (should (equal (plist-get result :main-behind) 1))
      (should (equal (plist-get result :remote-name) "origin"))
      (should (equal (plist-get result :remote-branch) "feature"))
      (should (equal (plist-get result :remote-ahead) 2))
      (should (equal (plist-get result :remote-behind) 0))
      (should (equal (plist-get result :symbols) "!↑⇡"))
      ;; :false from JSON false remains :false (not coerced to nil)
      (should (eq (plist-get result :is-main) :false))
      (should (eq (plist-get result :is-current) t))
      (should (eq (plist-get result :is-previous) :false)))))

(ert-deftest workset-test-parse-wt-entry-branch-only ()
  "Parse a branch-only wt entry (no worktree, no path, no working_tree, no remote)."
  (let* ((commit (make-hash-table :test #'equal))
         (main (make-hash-table :test #'equal))
         (entry (make-hash-table :test #'equal)))
    ...
    (let ((result (workset-worktree--parse-wt-entry entry)))
      ...
      (should-not (plist-get result :path))
      (should (equal (plist-get result :kind) "branch"))
      (should (equal (plist-get result :main-state) "integrated"))
      (should (equal (plist-get result :main-ahead) 0))
      (should (equal (plist-get result :main-behind) 5))
      (should (equal (plist-get result :symbols) "⊂⇣/"))
      ;; No workingTree key in hash → nil
      (should-not (plist-get result :working-tree))
      ;; No remote hash → nil for all remote fields
      (should-not (plist-get result :remote-name))
      (should-not (plist-get result :remote-branch))
      (should-not (plist-get result :remote-ahead))
      (should-not (plist-get result :remote-behind)))))
